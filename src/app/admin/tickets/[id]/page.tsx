    ...
    <div className="max-w-2xl space-y-4">
      <div className="card">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggleClosed} className="text-sm text-brand-500">
              {ticket.status === "CLOSED" ? "إعادة فتح" : "إغلاق التذكرة"}
            </button>
          </div>
        </div>
        <div className="space-y-3">
          {ticket.messages.map((m) => (
            <div key={m.id} className={`rounded-lg p-3 text-sm ${m.isStaff ? "bg-brand-50 dark:bg-brand-500/10" : "bg-black/5 dark:bg-white/5"}`}>
              <p className="mb-1 text-xs font-medium text-ink-800/50 dark:text-white/40">{m.isStaff ? "فريق الدعم" : m.user.name}</p>
              <p>{m.body}</p>
            </div>
          ))}
        </div>
      ...
      {ticket.status !== "CLOSED" && (
        <div className="card space-y-3">
          <textarea className="input min-h-[100px]" placeholder="اكتب الرد..." value={reply} onChange={(e) => setReply(e.target.value)} />
